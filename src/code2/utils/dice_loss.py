...
if __name__ == '__main__':
    x = torch.Tensor([[0.2068,  0.0313,  0.0716, -0.1760, -0.0989,  0.3676],
        [-0.0750,  0.1775,  0.2914,  0.0350, -0.3976,  0.7394],
        [-0.1099, -0.0680,  0.3033, -0.2801, -0.1535,  0.0324]])

    y = torch.tensor([[5], [5], [2]])
    loss_func = DiceLoss()

    flat_y = F.one_hot(y.reshape(-1), num_classes=6).float()
    flat_x = torch.nn.Softmax(dim=1)(x)
    loss = loss_func(x, y.reshape(-1))  # 损失函数更换为FocalLoss
    print(flat_y)
    print(flat_x)
    print(loss)

...
from dataset.category_id_map import category_id_to_lv2id
...
class MultiModalDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.tokenizer = BertTokenizer.from_pretrained(args.bert_dir, use_fast=True, cache_dir=args.bert_cache, add_special_tokens=False)
    ...
    def tokenize_text3(self, title: str, ocr_text: str, asr_text: str) -> tuple:
        texts = self.tokenizer.cls_token + title + self.tokenizer.sep_token + asr_text + self.tokenizer.sep_token \
                + ocr_text + self.tokenizer.sep_token
        encoded_texts = self.tokenizer(texts, max_length=self.bert_seq_length, padding='max_length', truncation=True)

        text_input_ids = torch.LongTensor(encoded_texts['input_ids'])
        text_mask = torch.LongTensor(encoded_texts['attention_mask'])
        text_token_type_ids = torch.zeros_like(text_input_ids)
        return text_input_ids, text_mask, text_token_type_ids
